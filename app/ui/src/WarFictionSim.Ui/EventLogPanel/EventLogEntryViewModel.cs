...
// 语义，现实时间仅在表现层附加显示），关键事件标记供置顶与样式使用。
...
public sealed class EventLogEntryViewModel
{
    ...
    public EventLogEntryViewModel(SimEventDto source)
    {
        Seq = source.Seq;
        Tick = source.Tick;
        ...
        GameTimeText = FormatGameTime(source.Tick);
    }
    ...
    /// <summary>游戏时间（mm:ss，按 20Hz tick 折算；纯展示）。</summary>
    public string GameTimeText { get; }

    private static string FormatGameTime(ulong tick)
    {
        double totalSeconds = tick / (double)TimeScales.DefaultTickHz;
